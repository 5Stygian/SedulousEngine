...
abstract class EngineApplication : IDisposable
{
	...
	/// Loads the builtin asset registry from the asset directory.
	/// Provides builtin:// protocol resources (primitives, materials, skies)
	...
	private void LoadBuiltinRegistry()
	{
		...
		mBuiltinRegistry = new ResourceRegistry("builtin", mAssetDirectory);
		if (mBuiltinRegistry.LoadFromFile(registryPath) case .Ok)
		{
			mResourceSystem.AddRegistry(mBuiltinRegistry);
		}
		else
		{
			delete mBuiltinRegistry;
			mBuiltinRegistry = null;
		}
	}
	...
	private void Cleanup()
	{
		...
		// Unregister builtin registry before resource system shutdown
		if (mBuiltinRegistry != null && mResourceSystem != null)
			mResourceSystem.RemoveRegistry(mBuiltinRegistry);
