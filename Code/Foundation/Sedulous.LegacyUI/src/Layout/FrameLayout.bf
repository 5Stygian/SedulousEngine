...
public class FrameLayout : ViewGroup
{
	...
	protected override void OnMeasure(MeasureSpec wSpec, MeasureSpec hSpec)
	{
		...
		for (int i = 0; i < ChildCount; i++)
		{
			...
			let childWSpec = MakeChildMeasureSpec(wSpec, Padding.TotalHorizontal + margin.TotalHorizontal, lp?.Width ?? Sedulous.LegacyUI.LayoutParams.WrapContent);
			let childHSpec = MakeChildMeasureSpec(hSpec, Padding.TotalVertical + margin.TotalVertical, lp?.Height ?? Sedulous.LegacyUI.LayoutParams.WrapContent);
