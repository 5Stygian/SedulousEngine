...
public class LinearLayout : ViewGroup
{
	...
	public override Sedulous.LegacyUI.LayoutParams CreateDefaultLayoutParams()
	{
	...
	private void MeasureVertical(MeasureSpec wSpec, MeasureSpec hSpec)
	{
		...
		for (int i = 0; i < ChildCount; i++)
		{
			...
			let childWSpec = MakeChildMeasureSpec(wSpec, Padding.TotalHorizontal + margin.TotalHorizontal, child.LayoutParams?.Width ?? Sedulous.LegacyUI.LayoutParams.WrapContent);
			let childHSpec = MakeChildMeasureSpec(hSpec, totalH + margin.TotalVertical, child.LayoutParams?.Height ?? Sedulous.LegacyUI.LayoutParams.WrapContent);
	...
	private void MeasureHorizontal(MeasureSpec wSpec, MeasureSpec hSpec)
	{
		...
		for (int i = 0; i < ChildCount; i++)
		{
			...
			let childWSpec = MakeChildMeasureSpec(wSpec, totalW + margin.TotalHorizontal, child.LayoutParams?.Width ?? Sedulous.LegacyUI.LayoutParams.WrapContent);
			let childHSpec = MakeChildMeasureSpec(hSpec, Padding.TotalVertical + margin.TotalVertical, child.LayoutParams?.Height ?? Sedulous.LegacyUI.LayoutParams.WrapContent);
